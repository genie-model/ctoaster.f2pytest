...
! CARBONATE CHEMISTRY TEST HARNESS -- ASSUMED INPUT + SIMPLIFIED OUTPUT (+ ADDITIONAL COMMENTING)
! **********************************************************************************************************************************
...
PROGRAM calc_carbchem_user
  ...
  USE gem_carbchem ! module containing primary subroutines used here
  USE gem_cmn      ! module contains array index strings and constants that are used here
  USE gem_util     ! module contains routines needed by gem_carbchem, and directly here e.g., fun_calc_isotope_fraction
  IMPLICIT NONE
  SAVE
  ...
  ! DEFINE VARIABLES
  ! -------------------------------------------------------- !
  ! these are arrays passed to/from subroutines
  ! array size variables (e.g., n_ocn) are parameters with values defined in gem_cmn
  ...
  ! these are all variables local to this routine (as opposed to variable defined in e.g., gem_cmn)
  real::loc_var
  REAL::loc_D
  ...
  ! -------------------------------------------------------- !
  ! in FORTRAN, we are grown-ups
  loc_ocn(:) = 0.0
  loc_carb(:) = 0.0
  loc_carbconst(:) = 0.0
  loc_carbalk(:) = 0.0
  loc_carbisor(:) = 0.0
  ! these determine the behavour of the calculation of pH in gem_carbchem
  ...
  ! NOTE: the interactive version is commented out in this version
  ! temperature
...
!!$  end if
  ...
  ! -------------------------------------------------------- !
  ! SOLVE pH IN SEAWATER
  ! -------------------------------------------------------- !
  ! NOTE: the various subroutines called take some inputs as 'in' only and some as 'inout'
  !       refer to the subroutines themselves in gem_carbchem
  ...
  ! calculate carbonate chemistry constants -- from gem_carbchem
  call sub_calc_carbconst( &
       ...
       & loc_carbconst(:)  &
       & )
  ! estimate Ca and borate etc concentrations from salinity -- all from gem_carbchem
  ...
  ! calculate carbonate chemistry system -- from gem_carbchem
  CALL sub_calc_carb(      &
       & loc_ocn(io_DIC),  &
       & loc_ocn(io_ALK),  &
       & loc_ocn(io_Ca),   &
       ...
       & loc_carbalk(:)    & 
       & )
  loc_DIC_13C = fun_calc_isotope_fraction(0.0,const_standards(11))*loc_ocn(io_DIC) ! from: gem_util
  call sub_calc_carb_r13C( &
       ...
       & loc_carbisor(:) &
       & )
  loc_DIC_14C = fun_calc_isotope_fraction(0.0,const_standards(12))*loc_ocn(io_DIC) ! from: gem_util
  ...
  ! PRINT RESULTS
  ! -------------------------------------------------------- !
  ! NOTE: shortened to some key indicative info
  PRINT*,'--- INPUTS ---'
